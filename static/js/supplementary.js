...
// === 보충이유서 목록 로드 함수 (async 추가 및 토큰 로직 포함) ===
async function loadComplaints() {
    ...
    memberListBody.innerHTML = `<tr><td colspan="${columnCount}" style="text-align: center; padding: 20px;">보충이유서 목록 로딩 중...</td></tr>`;
    ...
    const listApiEndpoint = '/api/supplementaries'; // 보충이유서 목록 API 경로
    ...
    try {
        ...
        if (summaries.length === 0) {
            memberListBody.innerHTML = `<tr><td colspan="${columnCount}" style="text-align: center;">표시할 보충이유서 내역 없음.</td></tr>`; return;
        }
